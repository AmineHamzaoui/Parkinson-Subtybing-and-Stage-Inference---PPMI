...
class AbstractSustain(ABC):
    ...
    def __init__(self,
        ...
        if self.use_parallel_startpoints:
            np_version = float(np.__version__.split(
                '.')[0] + '.' + np.__version__.split('.')[1])
            assert np_version >= 1.18, "numpy version must be >= 1.18 for parallelization to work properly."

            # pathos.multiprocessing.ParallelPool()
    ...
    def run_sustain_algorithm(self, plot=False, plot_format="png", **kwargs):
        ...
        for s in range(self.N_S_max):
            ...
            if pickle_filepath.exists():
                print("Found pickle file: " + pickle_filename_s +
                      ". Using pickled variables for " + str(s) + " subtype.")
                ...
                print("Failed to find pickle file: " + pickle_filename_s +
                      ". Running SuStaIn model for " + str(s) + " subtype.")
                ...
                ml_sequence_EM,     \
                    ...
                    ml_sequence_mat_EM, \
                    ml_f_mat_EM,        \
                    ml_likelihood_mat_EM = self._estimate_ml_sustain_model_nplus1_clusters(
                        self.__sustainData, ml_sequence_prev_EM, ml_f_prev_EM)  # self.__estimate_ml_sustain_model_nplus1_clusters(self.__data, ml_sequence_prev_EM, ml_f_prev_EM)
                ...
                ml_sequence,        \
                    ...
                    samples_sequence,   \
                    samples_f,          \
                    samples_likelihood = self._estimate_uncertainty_sustain_model(
                        self.__sustainData, seq_init, f_init)  # self.__estimate_uncertainty_sustain_model(self.__data, seq_init, f_init)
            ...
            ml_subtype,             \
                ...
                prob_subtype_stage = self.subtype_and_stage_individuals(
                    self.__sustainData, samples_sequence, samples_f, N_samples)  # self.subtype_and_stage_individuals(self.__data, samples_sequence, samples_f, N_samples)
            ...
            self._plot_biomarker_order = ml_sequence_EM[self._plot_subtype_order[0], :].astype(
                int)
            ...
            if plot:
                figs, ax = self._plot_sustain_model(
                    ...
                    save_path=Path(self.output_folder) /
                    f"{self.dataset_name}_subtype{s}_PVD.{plot_format}",
                    **kwargs
                )
                for fig in figs:
                    fig.show()

                ax0.plot(range(self.N_iterations_MCMC),
                         samples_likelihood, label="Subtype " + str(s+1))

        # save and show this figure after all subtypes have been calculcated
        if plot:
            ax0.legend(loc='upper right')
            fig0.tight_layout()
            fig0.savefig(Path(self.output_folder) /
                         f"MCMC_likelihoods.{plot_format}", bbox_inches='tight')
            fig0.show()
    ...
    def cross_validate_sustain_model(self, test_idxs, select_fold=[], plot=False):
        ...
        for fold in tqdm(select_fold, "Folds: ", Nfolds, position=0, leave=True):
            ...
            indx_train = np.array(
                [x for x in range(self.__sustainData.getNumSamples()) if x not in indx_test])
            ...
            for s in range(self.N_S_max):
                ...
                pickle_filename_fold_s = os.path.join(
                    pickle_dir, self.dataset_name + '_fold' + str(fold) + '_subtype' + str(s) + '.pickle')
                ...
                if pickle_filepath.exists():
                    ...
                    samples_likelihood_subj_test = self._evaluate_likelihood_setofsamples(
                    ...
                    ml_sequence_EM,         \
                        ...
                        ml_f_mat_EM,            \
                        ml_likelihood_mat_EM = self._estimate_ml_sustain_model_nplus1_clusters(
                            sustainData_train, ml_sequence_prev_EM, ml_f_prev_EM)
                    ...
                    ml_sequence,            \
                        ...
                        samples_f,              \
                        samples_likelihood = self._estimate_uncertainty_sustain_model(
                            sustainData_train, seq_init, f_init)

                    samples_likelihood_subj_test = self._evaluate_likelihood_setofsamples(
                        sustainData_test, samples_sequence, samples_f)

                    mean_likelihood_subj_test = np.mean(
                        samples_likelihood_subj_test, axis=1)
        ...
        if plot:
            ...
            df_loglike = pd.DataFrame(data=loglike_matrix, columns=[
                                      "Subtype " + str(i+1) for i in range(self.N_S_max)])
            ...
            fig.savefig(Path(self.output_folder) /
                        'Log_likelihoods_cv_folds.png')
            fig.show()
        ...
        for s in range(self.N_S_max):
            for fold in range(Nfolds):
                ...
                if fold == 0:
                    ...
                    mean_likelihood_subj_test_cval = np.concatenate(
                        (mean_likelihood_subj_test_cval, mean_likelihood_subj_test), axis=0)
    ...
    def combine_cross_validated_sequences(self, N_subtypes, N_folds, plot_format="png", **kwargs):
        ...
        pickle_filename_s = os.path.join(
            pickle_dir, self.dataset_name + '_subtype' + str(N_subtypes-1) + '.pickle')
        pickle_filepath = Path(pickle_filename_s)

        assert pickle_filepath.exists(), "Failed to find pickle file for full model with " + \
            str(N_subtypes) + " subtypes."
        ...
        for i in range(N_folds):
            ...
            pickle_filename_fold_s = os.path.join(
                pickle_dir, self.dataset_name + '_fold' + str(i) + '_subtype' + str(N_subtypes-1) + '.pickle')
            pickle_filepath = Path(pickle_filename_fold_s)
            ...
            for j in range(N_subtypes):
                for k in range(N_subtypes):
                    corr_mat[j, k] = stats.kendalltau(
                        ml_sequence_EM_full[j, :], ml_sequence_EM_i[k, :]).correlation
            set_full = []
            set_fold_i = []
            i_i, i_j = np.unravel_index(np.argsort(corr_mat.flatten())[
                                        ::-1], (N_subtypes, N_subtypes))
            ...
            index_set_full = np.argsort(set_full).astype(
                int)  # np.argsort(set_full)[::-1].astype(int)
            ...
            if i == 0:
                ...
                samples_sequence_cval = np.concatenate(
                    (samples_sequence_cval,    samples_sequence_i[iMax_vec, :, :]),  axis=2)
                samples_f_cval = np.concatenate(
                    (samples_f_cval,           samples_f_i[iMax_vec, :]),           axis=1)
        ...
        # order of biomarkers in each subtypes' positional variance diagram
        plot_biomarker_order = ml_sequence_EM_full[plot_subtype_order[0], :].astype(
            int)
    ...
    def subtype_and_stage_individuals(self, sustainData, samples_sequence, samples_f, N_samples):
        ...
        select_samples = np.round(np.linspace(
            0, n_iterations_MCMC - 1, N_samples))
        ...
        for i in range(N_samples):
            ...
            _,                  \
                ...
                total_prob_stage,   \
                total_prob_subtype, \
                total_prob_subtype_stage = self._calculate_likelihood(
                    sustainData, this_S, this_f)

            total_prob_subtype = total_prob_subtype.reshape(
                len(total_prob_subtype), N_S)
            total_prob_subtype_norm = total_prob_subtype / \
                np.tile(np.sum(total_prob_subtype, 1).reshape(
                    len(total_prob_subtype), 1),        (1, N_S))
            total_prob_stage_norm = total_prob_stage / np.tile(np.sum(total_prob_stage, 1).reshape(
                len(total_prob_stage), 1),          (1, nStages + 1))  # removed total_prob_subtype
            ...
            total_prob_subtype_stage_norm = total_prob_subtype_stage / \
                np.tile(np.sum(np.sum(total_prob_subtype_stage, 1, keepdims=True), 2).reshape(
                    nSamples, 1, 1), (1, nStages + 1, N_S))

            prob_subtype_stage = (i / (i + 1.) * prob_subtype_stage) + \
                (1. / (i + 1.) * total_prob_subtype_stage_norm)
            prob_subtype = (i / (i + 1.) * prob_subtype) + \
                (1. / (i + 1.) * total_prob_subtype_norm)
            prob_stage = (i / (i + 1.) * prob_stage) + \
                (1. / (i + 1.) * total_prob_stage_norm)
        ...
        for i in range(nSamples):
            ...
            if (np.sum(np.isnan(this_prob_subtype)) == 0):
                ...
                this_subtype = np.where(
                    this_prob_subtype == np.max(this_prob_subtype))
            ...
            this_prob_stage = np.squeeze(
                prob_subtype_stage[i, :, int(ml_subtype[i])])

            if (np.sum(np.isnan(this_prob_stage)) == 0):
                # this_stage =
                this_stage = np.where(
                    this_prob_stage == np.max(this_prob_stage))
    ...
    def _estimate_ml_sustain_model_nplus1_clusters(self, sustainData, ml_sequence_prev, ml_f_prev):
        ...
        if N_S == 1:
            ...
            _, _, _, p_sequence, _ = self._calculate_likelihood(
                sustainData, ml_sequence_prev, ml_f_prev)

            ml_sequence_prev = ml_sequence_prev.reshape(
                ml_sequence_prev.shape[0], ml_sequence_prev.shape[1])
            p_sequence = p_sequence.reshape(p_sequence.shape[0], N_S - 1)
            p_sequence_norm = p_sequence / \
                np.tile(np.sum(p_sequence, 1).reshape(
                    len(p_sequence), 1), (N_S - 1))

            # Assign individuals to a subtype (cluster) based on the previous model
            # np.zeros((len(data_local), 1))
            ml_cluster_subj = np.zeros((sustainData.getNumSamples(), 1))
            # range(len(data_local)):
            for m in range(sustainData.getNumSamples()):
                ...
                # FIXME: should check this always works, as it differs to the Matlab code, which treats ix as an array
                ml_cluster_subj[m] = ix

            ml_likelihood = -np.inf
            for ix_cluster_split in range(N_S - 1):
                this_N_cluster = sum(
                    ml_cluster_subj == int(ix_cluster_split + 1))
                ...
                if this_N_cluster > 1:
                    ...
                    print('Splitting cluster',
                          ix_cluster_split + 1, 'of', N_S - 1)
                    ix_i = (ml_cluster_subj == int(ix_cluster_split + 1)
                            ).reshape(sustainData.getNumSamples(), )
                    sustainData_i = sustainData.reindex(ix_i)

                    print(' + Resolving 2 cluster problem')
                    this_ml_sequence_split, _, _, _, _, _ = self._find_ml_split(
                        sustainData_i)

                    # Use the two subtype model combined with the other subtypes to
                    # inititialise the fitting of the next SuStaIn model in the
                    # hierarchy
                    # have to copy or changes will be passed to ml_sequence_prev
                    this_seq_init = ml_sequence_prev.copy()

                    # replace the previous sequence with the first (row index zero) new sequence
                    this_seq_init[ix_cluster_split] = (this_ml_sequence_split[0]).reshape(
                        this_ml_sequence_split.shape[1])

                    # add the second new sequence (row index one) to the stack of sequences,
                    # so that you now have N_S sequences instead of N_S-1
                    this_seq_init = np.hstack(
                        (this_seq_init.T, this_ml_sequence_split[1])).T
                    ...
                    this_ml_sequence,       \
                        ...
                        this_ml_f_mat,          \
                        this_ml_likelihood_mat = self._find_ml_mixture(
                            sustainData, this_seq_init, this_f_init)
                    ...
                    print(
                        f'Cluster {ix_cluster_split + 1} of {N_S - 1} too small for subdivision')
    ...
    def _find_ml(self, sustainData):
        ...
        pool_output_list = self.pool.map(
            partial_iter, seed_sequences.spawn(self.N_startpoints))

        if ~isinstance(pool_output_list, list):
            pool_output_list = list(pool_output_list)

        # np.zeros((1, self.stage_zscore.shape[1], self.N_startpoints))
        ml_sequence_mat = np.zeros(
            (1, sustainData.getNumStages(), self.N_startpoints))
    ...
    def _find_ml_split(self, sustainData):
        ...
        ml_sequence_mat = np.zeros(
            (N_S, sustainData.getNumStages(), self.N_startpoints))
        ...
        # ugly bit of code to get first index where likelihood is maximum
    ...
    def _find_ml_split_iteration(self, sustainData, seed_seq):
        ...
        for s in range(N_S):
            index_s = cluster_assignment.reshape(
                cluster_assignment.shape[0], ) == (s + 1)
            temp_sustainData = sustainData.reindex(index_s)

            temp_seq_init = self._initialise_sequence(sustainData, rng)
            seq_init[s, :], _, _, _, _, _ = self._perform_em(
                temp_sustainData, temp_seq_init, [1], rng)
        ...
        this_ml_sequence, \
            this_ml_f, \
            this_ml_likelihood, _, _, _ = self._perform_em(
    ...
    def _find_ml_mixture(self, sustainData, seq_init, f_init):
        ...
        partial_iter = partial(
            self._find_ml_mixture_iteration, sustainData, seq_init, f_init)
        seed_sequences = np.random.SeedSequence(self.global_rng.integers(1e10))
        pool_output_list = self.pool.map(
            partial_iter, seed_sequences.spawn(self.N_startpoints))
    ...
    def _find_ml_mixture_iteration(self, sustainData, seq_init, f_init, seed_seq):
        ...
        ml_sequence,        \
            ...
            samples_sequence,   \
            samples_f,          \
            samples_likelihood = self._perform_em(
                sustainData, seq_init, f_init, rng)
    ...
    def _perform_em(self, sustainData, current_sequence, current_f, rng):
        ...
        current_likelihood, _, _, _, _ = self._calculate_likelihood(
            sustainData, current_sequence, current_f)
        ...
        samples_sequence[0, :, :] = current_sequence.reshape(
            current_sequence.shape[1], current_sequence.shape[0])
        ...
        while terminate == 0:
            ...
            candidate_sequence,     \
                candidate_f,            \
                candidate_likelihood = self._optimise_parameters(
                    sustainData, current_sequence, current_f, rng)

            HAS_converged = np.fabs((candidate_likelihood - current_likelihood) /
                                    max(candidate_likelihood, current_likelihood)) < 1e-6
            if HAS_converged:
            ...
            samples_sequence[iteration, :, :] = current_sequence.T.reshape(
                current_sequence.T.shape[0], N_S)
    ...
    def _calculate_likelihood(self, sustainData, S, f):
        ...
        for s in range(N_S):
            # self.__calculate_likelihood_stage_linearzscoremodel_approx(data_local, S[s])
    ...
    def _estimate_uncertainty_sustain_model(self, sustainData, seq_init, f_init):
        ...
        ml_sequence,        \
            ...
            samples_sequence,   \
            samples_f,          \
            samples_likelihood = self._perform_mcmc(
                sustainData, seq_init, f_init, self.N_iterations_MCMC, seq_sigma_opt, f_sigma_opt)
    ...
    def _optimise_mcmc_settings(self, sustainData, seq_init, f_init):
        ...
        for i in range(n_passes_optimisation):
            ...
            samples_position_currentpass = np.zeros(
                samples_sequence_currentpass.shape)
            for s in range(N_S):
                for sample in range(n_iterations_MCMC_optimisation):
                    temp_seq = samples_sequence_currentpass[s, :, sample]
                    temp_inv = np.array(
                        [0] * samples_sequence_currentpass.shape[1])
                    temp_inv[temp_seq.astype(int)] = np.arange(
                        samples_sequence_currentpass.shape[1])
                    samples_position_currentpass[s, :, sample] = temp_inv

            # np.std is different to Matlab std, which normalises to N-1 by default
            seq_sigma_currentpass = np.std(
                samples_position_currentpass, axis=2, ddof=1)
            seq_sigma_currentpass[seq_sigma_currentpass <
                                  0.01] = 0.01  # magic number
    ...
    def _evaluate_likelihood_setofsamples(self, sustainData, samples_sequence, samples_f):
        ...
        if n_total >= 1e6:
            ...
            N_samples = n_total
        select_samples = np.round(np.linspace(
            0, n_total - 1, N_samples)).astype(int)
        ...
        for i in range(n_iterations):
            ...
            _, likelihood_sample_subj, _, _, _ = self._calculate_likelihood(
                sustainData, S, f)
    ...
    @abstractmethod
    # up to the implementations to define exact number of params here
    def subtype_and_stage_individuals_newData(self):
        pass
    ...
    def check_biomarker_colours(biomarker_colours, biomarker_labels):
        if isinstance(biomarker_colours, dict):
            ...
            assert all(i in biomarker_labels for i in biomarker_colours.keys(
            )), "A label doesn't match!"
            # Check each colour exists
            assert all(mcolors.is_color_like(
                i) for i in biomarker_colours.values()), "A proper colour wasn't given!"
            ...
            # Check each colour exists
            assert all(mcolors.is_color_like(i)
                       for i in biomarker_colours), "A proper colour wasn't given!"
            # Check right number of colours given
            assert len(biomarker_colours) == len(
                biomarker_labels), "The number of colours and labels do not match!"
            # Turn list of colours into a label:colour mapping
            biomarker_colours = {k: v for k, v in zip(
                biomarker_labels, biomarker_colours)}
        else:
            raise TypeError(
                "A dictionary mapping label:colour or list/tuple of colours must be given!")
