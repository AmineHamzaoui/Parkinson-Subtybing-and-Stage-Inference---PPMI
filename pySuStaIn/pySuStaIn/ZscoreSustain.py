...
class ZscoreSustain(AbstractSustain):
    ...
    def __init__(self,
        ...
        assert (len(biomarker_labels) ==
                N), "number of labels should match number of biomarkers"

        # np.array([y for x in Z_vals.T for y in x])
        ...
        # np.array([y for x in IX_vals.T for y in x])
        stage_biomarker_index = IX_vals.T.flatten()
        ...
        for i in range(N):
            ...
            for j in range(len(possible_biomarkers)):
                ...
                if not np.any(this_biomarkers):
                    ...
                    this_min_stage_zscore = min(
                        self.stage_zscore[this_biomarkers])
                if (this_min_stage_zscore):
                    temp = ((this_biomarkers.astype(
                        int) + (self.stage_zscore == this_min_stage_zscore).astype(int)) == 2).T
    ...
    def _calculate_likelihood_stage(self, sustainData, S):
        ...
        for i in range(B):
            ...
            event_value = np.concatenate([[self.min_biomarker_zscore[i]], self.stage_zscore[self.stage_biomarker_index == b], [
        ...
        stage_value = 0.5 * \
            point_value[:, :point_value.shape[1] - 1] + \
            0.5 * point_value[:, 1:]
        ...
        x = (sustainData.data[:, :, None] -
             stage_value) / sigmat[None, :, None]
        p_perm_k = coeff + \
            np.sum(factor[None, :, None] - 0.5 * np.square(x), 1)
        p_perm_k = np.exp(p_perm_k)
    ...
    def _optimise_parameters(self, sustainData, S_init, f_init, rng):
        ...
        p_perm_k_norm = p_perm_k_weighted / \
            np.sum(p_perm_k_weighted + 1e-250, axis=(1, 2), keepdims=True)
        f_opt = (np.squeeze(sum(sum(p_perm_k_norm))) /
                 sum(sum(sum(p_perm_k_norm)))).reshape(N_S, 1, 1)
        f_val_mat = np.tile(f_opt, (1, N + 1, M))
        f_val_mat = np.transpose(f_val_mat, (2, 1, 0))
        # this will produce different random numbers to Matlab
        ...
        for s in order_seq:
            ...
            for i in order_bio:
                ...
                current_location[current_sequence.astype(
                    int)] = np.arange(len(current_sequence))
                ...
                selected_biomarker = self.stage_biomarker_index[0,
                                                                selected_event]
                ...
                if np.any(min_filter):
                    ...
                    min_zscore_bound_event = events[((self.stage_zscore[0] == min_zscore_bound).astype(
                        int) + (self.stage_biomarker_index[0] == selected_biomarker).astype(int)) == 2]
                ...
                if np.any(max_filter):
                    ...
                    max_zscore_bound_event = events[((self.stage_zscore[0] == max_zscore_bound).astype(
                        int) + (self.stage_biomarker_index[0] == selected_biomarker).astype(int)) == 2]
                ...
                if move_event_to_lower_bound == move_event_to_upper_bound:
                    ...
                    possible_positions = np.arange(
                        move_event_to_lower_bound, move_event_to_upper_bound)
                possible_sequences = np.zeros((len(possible_positions), N))
                possible_likelihood = np.zeros((len(possible_positions), 1))
                possible_p_perm_k = np.zeros(
                    (M, N + 1, len(possible_positions)))
                for index in range(len(possible_positions)):
                    ...
                    # this is different to the Matlab version, which call current_sequence(move_event_from) = []
                    current_sequence = np.delete(
                        current_sequence, move_event_from, 0)
                    new_sequence = np.concatenate([current_sequence[np.arange(move_event_to)], [
                                                  selected_event], current_sequence[np.arange(move_event_to, N - 1)]])
                    possible_sequences[index, :] = new_sequence

                    possible_p_perm_k[:, :, index] = self._calculate_likelihood_stage(
                        sustainData, new_sequence)

                    p_perm_k[:, :, s] = possible_p_perm_k[:, :, index]
                    total_prob_stage = np.sum(p_perm_k * f_val_mat, 2)
                    total_prob_subj = np.sum(total_prob_stage, 1)
                    possible_likelihood[index] = np.sum(
                        np.log(total_prob_subj + 1e-250))

                possible_likelihood = possible_likelihood.reshape(
                    possible_likelihood.shape[0])
                max_likelihood = max(possible_likelihood)
                this_S = possible_sequences[possible_likelihood ==
                                            max_likelihood, :]
                this_S = this_S[0, :]
                S_opt[s] = this_S
                this_p_perm_k = possible_p_perm_k[:, :,
                                                  possible_likelihood == max_likelihood]
    ...
    def _perform_mcmc(self, sustainData, seq_init, f_init, n_iterations, seq_sigma, f_sigma):
        ...
        # don't need to copy as we don't write to 0 index
        samples_sequence[:, :, 0] = seq_init
        samples_f[:, 0] = f_init

        # Reduce frequency of tqdm update to 0.1% of total for larger iteration numbers
        tqdm_update_iters = int(
            n_iterations/1000) if n_iterations > 100000 else None
        ...
        for i in tqdm(range(n_iterations), "MCMC Iteration", n_iterations, miniters=tqdm_update_iters):
            if i > 0:
                ...
                for s in seq_order:
                    move_event_from = int(
                        np.ceil(N * self.global_rng.random())) - 1
                    ...
                    weight = AbstractSustain.calc_coeff(
                        this_seq_sigma) * AbstractSustain.calc_exp(distance, 0., this_seq_sigma)
                    weight /= np.sum(weight)
                    # FIXME: difficult to check this because random.choice is different to Matlab randsample
                    index = self.global_rng.choice(
                        range(len(possible_positions)), 1, replace=True, p=weight)
                ...
                new_f = samples_f[:, i - 1] + f_sigma * \
                    self.global_rng.standard_normal()
    ...
    def plot_positional_var(samples_sequence, samples_f, n_samples, Z_vals, biomarker_labels=None, ml_f_EM=None, cval=False, subtype_order=None, biomarker_order=None, title_font_size=12, stage_font_size=10, stage_label='SuStaIn Stage', stage_rot=0, stage_interval=1, label_font_size=10, label_rot=0, cmap="original", biomarker_colours=None, figsize=None, subtype_titles=None, separate_subtypes=False, save_path=None, save_kwargs={}):
        ...
        if cmap == "original":
            ...
            colour_mat = np.array([[1, 0, 0], [1, 0, 1], [0, 0, 1], [
                                  0.5, 0, 1], [0, 1, 1], [0, 1, 0.5]])[:N_z]
            # We only have up to 5 default colours, so double-check
            if colour_mat.shape[0] > N_z:
                raise ValueError(
                    f"Colours are only defined for {len(colour_mat)} z-scores!")
        else:
        ...
        for i in range(subtype_loops):
            ...
            for j in range(total_axes):
                ...
                confus_matrix = (this_samples_sequence == np.arange(
                    N)[:, None, None]).sum(1) / this_samples_sequence.shape[0]
                ...
                for j, z in enumerate(zvalues):
                    ...
                    confus_matrix_zscore = confus_matrix[(
                        stage_zscore == z)[0]]
                    # Subtract the certainty for this colour
                    confus_matrix_c[
                        np.ix_(
                            stage_biomarker_index[(stage_zscore == z)[
                                0]], range(N),
                            alter_level
                        )
                    ] -= np.tile(
                        confus_matrix_zscore.reshape(
                            (stage_zscore == z).sum(), N, 1),
                        (1, 1, alter_level.sum())
                    )
                if subtype_titles is not None:
                ...
                ax.set_xticklabels(
                    stage_ticks+1, fontsize=stage_font_size, rotation=stage_rot)
                # Add the yticks and labels
                ax.set_yticks(np.arange(N_bio))
                # Add biomarker labels to LHS of every row only
                if (i % ncols) == 0:
                    ax.set_yticklabels(
                        biomarker_labels, ha='right', fontsize=label_font_size, rotation=label_rot)
                    # Set biomarker label colours
                    for tick_label in ax.get_yticklabels():
                        tick_label.set_color(
                            biomarker_colours[tick_label.get_text()])
                else:
                    ax.set_yticklabels([])
    ...
    def test_sustain(cls, n_biomarkers, n_samples, n_subtypes,
        ...
        ground_truth_stages_control = np.zeros(
            (int(np.round(n_samples * 0.25)), 1))
        ground_truth_stages_other = np.random.randint(
            1, N_stages+1, (int(np.round(n_samples * 0.75)), 1))
        ground_truth_stages = np.vstack(
            (ground_truth_stages_control, ground_truth_stages_other)).astype(int)
    ...
    def generate_random_model(Z_vals, N_S, seed=None):
        ...
        stage_biomarker_index = np.tile(
            np.arange(num_biomarkers), (num_zscores,))
        # [np.newaxis, :]
        stage_biomarker_index = stage_biomarker_index[IX_select]
            ...
            for i in range(N):
                ...
                for j in possible_biomarkers:
                    ...
                    if not np.any(this_biomarkers):
                        ...
                        this_min_stage_zscore = np.min(
                            stage_zscore[this_biomarkers])
                ...
                this_index = np.ceil(np.random.rand() *
    ...
    def generate_data(subtypes, stages, gt_ordering, Z_vals, Z_max):
        ...
        for s in range(N_S):
            ...
            for i in range(B):
                b = possible_biomarkers[i]
                event_location = np.concatenate(
                    [[0], S_inv[(stage_biomarker_index == b)[0]], [N]])
                ...
                for j in range(len(event_location)-1):
                    ...
                    if j == 0:  # FIXME: nasty hack to get Matlab indexing to match up - necessary here because indices are used for linspace limits
                        index = np.arange(
                            event_location[j], event_location[j+1]+2)
                        stage_value[i, index, s] = np.linspace(
                            event_value[j], event_value[j+1], event_location[j+1]-event_location[j]+2)
                    else:
                        index = np.arange(
                            event_location[j] + 1, event_location[j + 1] + 2)
                        stage_value[i, index, s] = np.linspace(
        ...
        data = data_denoised + \
            norm.ppf(np.random.rand(B, M).T) * \
            np.tile(std_biomarker_zscore, (M, 1))
